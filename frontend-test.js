...
async function testFrontendIntegration() {
  ...
  try {
    ...
    if (!encryptResponse.ok) {
      throw new Error(
        `Frontend encryption API failed with status ${encryptResponse.status}`
      );
    }
    ...
    const decryptWithKeyResponse = await fetch(
      "http://localhost:3005/api/decrypt",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!decryptWithKeyResponse.ok) {
      throw new Error(
        `Frontend decryption API with key failed with status ${decryptWithKeyResponse.status}`
      );
    }
    ...
    if (!decryptWithRSAResponse.ok) {
      throw new Error(
        `Frontend decryption API with RSA failed with status ${decryptWithRSAResponse.status}`
      );
    }
    ...
    console.log(
      `Decrypted message: "${decryptWithRSAResult.decryptedMessage}"`
    );
    ...
    console.log(
      `Decrypted (Hill key): "${decryptWithKeyResult.decryptedMessage.replace(
        /X+$/,
        ""
      )}"`
    );
    console.log(
      `Decrypted (RSA): "${decryptWithRSAResult.decryptedMessage.replace(
        /X+$/,
        ""
      )}"`
    );

    const cleanedHillDecrypted = decryptWithKeyResult.decryptedMessage.replace(
      /X+$/,
      ""
    );
    const cleanedRSADecrypted = decryptWithRSAResult.decryptedMessage.replace(
      /X+$/,
      ""
    );
