...
async function testEncryptDecryptIntegration() {
  ...
  try {
    ...
    if (!encryptResponse.ok) {
      throw new Error(
        `Encryption failed with status ${encryptResponse.status}`
      );
    }
    ...
    const decryptWithKeyResponse = await fetch(
      "http://localhost:5001/api/decrypt",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!decryptWithKeyResponse.ok) {
      throw new Error(
        `Decryption with key failed with status ${decryptWithKeyResponse.status}`
      );
    }
    ...
    if (!decryptWithRSAResponse.ok) {
      throw new Error(
        `Decryption with RSA failed with status ${decryptWithRSAResponse.status}`
      );
    }
    ...
    console.log(
      `Decrypted (Hill key): "${decryptWithKeyResult.decryptedMessage}"`
    );
